...
INTERFACES:=$(shell go list -f '{{range .GoFiles}}{{ $$.Dir }}/{{.}} {{end}}' ./interfaces | sed -e 's@$(CURRENT_DIR)/@@g' )

.PHONY: all devtools deps mocks test build clean rpm
...
## just an alias to build the mocks
mocks: .godeps/.mocks_generated

## this is an "empty target"
## http://www.gnu.org/software/make/manual/make.html#Empty-Targets
.godeps/.mocks_generated: $(BIN)/mockery $(INTERFACES)
	$(GVP) in $(BIN)/mockery -dir=interfaces -case=underscore -all -inpkg
	@touch $@

## run tests
test: $(BIN)/ginkgo mocks
	$(GVP) in $(BIN)/ginkgo -r
